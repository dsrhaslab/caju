...
int main(int argc, char* argv[]) {
    ...
    SPDLOG_LOGGER_TRACE(logger, "open(\"file.txt\", O_CREAT | O_WRONLY | O_TRUNC, 0666) -> Status: {}, File_Id fd: {}", (int) out_open.state, ((FileIdInt*) file_id.get())->fd);

    ///*
    Status<ssize_t> out_write = tier.write("lmao", 4, *file_id);
    SPDLOG_LOGGER_TRACE(logger, "write(\"lmao\", 4, {}) -> Status: {}, Return: {}", ((FileIdInt*) file_id.get())->fd, (int) out_write.state, out_write.return_value);
    //*/

    Status<int> out_close = tier.close(*file_id);
    SPDLOG_LOGGER_TRACE(logger, "close(File_Id fd: {}) -> Status: {}, Return: {}", ((FileIdInt*) file_id.get())->fd, (int) out_close.state, out_close.return_value);
    ...
    SPDLOG_LOGGER_TRACE(logger, "open(\"file.txt\", O_CREAT | O_RDONLY, 0666) -> Status: {}, File_Id fd: {}", (int) out_open2.state, ((FileIdInt*) file_id2.get())->fd);

    char buf[5] = "";
    Status<ssize_t> out_read = tier.read(buf, 4, *file_id2);
    buf[4] = '\0';
    SPDLOG_LOGGER_TRACE(logger, "read(Count: {}) -> Status: {}, Return: {}, Buf: {}", 4, (int) out_read.state, out_read.return_value, buf);

    Status<int> out_close2 = tier.close(*file_id2);
    SPDLOG_LOGGER_TRACE(logger, "close(File_Id fd: {}) -> Status: {}, Return: {}", ((FileIdInt*) file_id2.get())->fd, (int) out_close2.state, out_close2.return_value);
